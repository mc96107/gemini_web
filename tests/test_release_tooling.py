...
def test_recombine_execution():
    ...
    if os.path.exists("opencode_agent_release.py"):
        os.remove("opencode_agent_release.py")
    
    # Run script
    result = subprocess.run([sys.executable, "scripts/recombine.py"], capture_output=True, text=True)
    assert result.returncode == 0
    assert os.path.exists("opencode_agent_release.py")
    
    # Check syntax of output
    result_syntax = subprocess.run([sys.executable, "-m", "py_compile", "opencode_agent_release.py"], capture_output=True, text=True)
