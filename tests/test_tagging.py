...
def agent(tmp_path):
    ...
    working_dir.mkdir()
    agent = OpenCodeAgent(working_dir=str(working_dir))
    return agent
...
async def test_tag_filtering(agent):
    ...
    agent.user_data[user_id] = {
        "active_session": None,
        "sessions": [s1, s2],
        "session_tags": {s1: ["work"], s2: ["personal"]},
    }
    ...
    mock_stdout = json.dumps(
        [
            ...
            },
            {
                "id": s2,
                "title": "S2",
                "updated": 1771684009401,
                "created": 1771684005294,
            },
        ]
    ).encode()
    ...
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        sessions_data = await agent.get_user_sessions(
            user_id, tags=["work", "personal"]
        )
        filtered = sessions_data["pinned"] + sessions_data["history"]
