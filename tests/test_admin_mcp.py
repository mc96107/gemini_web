...
def test_admin_mcp_list():
    ...
    # Register and Mock admin login
    app.state.user_manager.register_user("admin", "password", role="admin")
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            stdout="✓ web-inspector: npx -y mcp-web-inspector (stdio) - Connected",
...
def test_admin_mcp_toggle():
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            stdout="Enabled server web-inspector", check_returncode=MagicMock()
        )

        response = client.post(
            "/admin/mcp/toggle", json={"name": "web-inspector", "enabled": True}
        )
        assert response.status_code == 200
