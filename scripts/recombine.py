...
def recombine():
    ...
    if os.path.exists(templates_dir):
        for filename in os.listdir(templates_dir):
            if filename.endswith(".html"):
                templates[filename] = get_file_content(
    ...
    def strip_local_imports(code):
        ...
        for line in lines:
            if not in_block:
                ...
                is_local = (
                    re.match(r"^\s*(from|import) (app\.|core|services|routers)", line)
                    or "from app.core import config" in line
                    or "from app.core.patterns" in line
                )
    ...
    patterns_code = patterns_code.replace(
        'os.path.join(os.path.dirname(__file__), "../../data/patterns.json")',
        'os.path.join(os.getcwd(), "data", "patterns.json")',
    )
    patterns_code += "\n# Ensure data directory exists\nos.makedirs(os.path.dirname(PATTERNS_FILE), exist_ok=True)\n"

    user_manager_code = strip_local_imports(
        get_file_content("app/services/user_manager.py")
    )
    auth_service_code = clean_config_ref(
        strip_local_imports(get_file_content("app/services/auth_service.py"))
    )
    llm_service_code = clean_config_ref(
        strip_local_imports(get_file_content("app/services/llm_service.py"))
    )
    sync_service_code = strip_local_imports(
        get_file_content("app/services/pattern_sync_service.py")
    )
    conversion_service_code = clean_config_ref(
        strip_local_imports(get_file_content("app/services/conversion_service.py"))
    )
    ...
    auth_router_code = clean_config_ref(
        strip_local_imports(get_file_content("app/routers/auth.py"))
    )
    ...
    chat_router_code = clean_config_ref(
        strip_local_imports(get_file_content("app/routers/chat.py"))
    )
    admin_router_code = clean_config_ref(
        strip_local_imports(get_file_content("app/routers/admin.py"))
    )
    ...
    combined.append(
        auth_router_code.replace("router = APIRouter()", "").replace(
            "@router.", "@auth_router."
        )
    )
    combined.append("\n")
    ...
    combined.append(
        chat_router_code.replace("router = APIRouter()", "").replace(
            "@router.", "@chat_router."
        )
    )
    combined.append("\n")
    ...
    combined.append(
        admin_router_code.replace("router = APIRouter()", "").replace(
            "@router.", "@admin_router."
        )
    )
    combined.append("\n")
...
"""
    ...
    main_clean = re.sub(
        r'@app\.get\("/favicon\.ico".*?return FileResponse\(.*?\)\n+',
        "",
        main_clean,
        flags=re.DOTALL,
    )
    main_clean = re.sub(
        r'@app\.get\("/sw\.js".*?return FileResponse\(.*?\)\n+',
        "",
        main_clean,
        flags=re.DOTALL,
    )
    main_clean = re.sub(
        r'@app\.get\("/manifest\.json".*?return FileResponse\(.*?\)\n+',
        "",
        main_clean,
        flags=re.DOTALL,
    )
    ...
    main_clean = main_clean.replace(
        "app.include_router(auth.router)", "app.include_router(auth_router)"
    )
    main_clean = main_clean.replace(
        "app.include_router(chat.router)", "app.include_router(chat_router)"
    )
    main_clean = main_clean.replace(
        "app.include_router(admin.router)", "app.include_router(admin_router)"
    )
    ...
    if 'if __name__ == "__main__":' in main_clean:
        main_clean = main_clean.replace(
            'if __name__ == "__main__":', custom_logic + '\nif __name__ == "__main__":'
        )
    else:
    ...
    # Output
    output_file = "opencode_agent_release.py"
